// docs/settings.js
// Eingebaute Presets + aktuelle Config + Overlay-UI + Export/Import (CF|cfg-v1.0|…)
import {
  TILE as D_TILE, COLS as D_COLS, ROWS as D_ROWS, TICK_MS as D_TICK_MS,
  ATTACK_PERIOD as D_AP, THREAT_INC as D_TI, THREAT_DROP_AFTER as D_TD,
...
// Standard-Config aus config.js
function defaults() {
  return {
    TILE: D_TILE, COLS: D_COLS, ROWS: D_ROWS, TICK_MS: D_TICK_MS,
    ATTACK_PERIOD: D_AP, THREAT_INC: D_TI, THREAT_DROP_AFTER: D_TD,
...
export function onConfigChange(fn){ listeners.push(fn); }
...
function sanitizeCfg(cfg){
  const d = defaults();
  const out = defaults();
  // wir übernehmen nur bekannte Felder (keine GRID/TILE-Änderungen hier!)
  out.TICK_MS = typeof cfg.TICK_MS==='number' ? clampInt(cfg.TICK_MS) || d.TICK_MS : d.TICK_MS;
  out.ATTACK_PERIOD = typeof cfg.ATTACK_PERIOD==='number' ? clampInt(cfg.ATTACK_PERIOD)||d.ATTACK_PERIOD : d.ATTACK_PERIOD;
  out.THREAT_INC = typeof cfg.THREAT_INC==='number' ? +cfg.THREAT_INC : d.THREAT_INC;
  out.THREAT_DROP_AFTER = typeof cfg.THREAT_DROP_AFTER==='number' ? clampInt(cfg.THREAT_DROP_AFTER)||d.THREAT_DROP_AFTER : d.THREAT_DROP_AFTER;
  out.ATTACK_BASE_MIN = typeof cfg.ATTACK_BASE_MIN==='number' ? clampInt(cfg.ATTACK_BASE_MIN)||d.ATTACK_BASE_MIN : d.ATTACK_BASE_MIN;
  out.ATTACK_BASE_RAND = typeof cfg.ATTACK_BASE_RAND==='number' ? clampInt(cfg.ATTACK_BASE_RAND)||d.ATTACK_BASE_RAND : d.ATTACK_BASE_RAND;
  out.ATTACK_THREAT_FACTOR = typeof cfg.ATTACK_THREAT_FACTOR==='number' ? +cfg.ATTACK_THREAT_FACTOR : d.ATTACK_THREAT_FACTOR;
  if (cfg.START_RES) {
    out.START_RES.wood = clampInt(cfg.START_RES.wood ?? d.START_RES.wood);
    out.START_RES.metal = clampInt(cfg.START_RES.metal ?? d.START_RES.metal);
    out.START_RES.food  = clampInt(cfg.START_RES.food  ?? d.START_RES.food);
  }
  if (cfg.BUILD_DEF) {
    ...
    for (const [k,v] of Object.entries(cfg.BUILD_DEF)) {
      ...
      if (v.cost) for (const c of Object.keys(v.cost)) if (t.cost?.[c]!=null) t.cost[c]=clampInt(v.cost[c]);
      if (v.out)  for (const o of Object.keys(v.out)) if (t.out?.[o]!=null) t.out[o]=clampInt(v.out[o]);
      if (v.power?.prod!=null && t.power) t.power.prod = clampInt(v.power.prod);
      if (v.power?.need!=null && t.power) t.power.need = clampInt(v.power.need);
      if (v.defense!=null) t.defense = clampInt(v.defense);
      if (v.up!=null) t.up = +v.up;
      if (v.max!=null) t.max = clampInt(v.max)||1;
    }
  }
    ...
    <div id="cfg-body">
      <div>
        ...
        <select id="cfg-preset">
          ${Object.keys(PRESETS).map(n=>`<option value="${n}" ${current.name===n?'selected':''}>${n}</option>`).join("")}
        </select>
      </div>
      <div id="cfg-grid">
        <div><span class="tag">Threat +/s:</span> ${cfg.THREAT_INC}</div>
        <div><span class="tag">Angriff alle ~s:</span> ${cfg.ATTACK_PERIOD}</div>
        <div><span class="tag">Start Holz:</span> ${cfg.START_RES.wood}</div>
        <div><span class="tag">Start Metall:</span> ${cfg.START_RES.metal}</div>
        <div><span class="tag">Start Nahrung:</span> ${cfg.START_RES.food}</div>
      </div>
      ...
      <details>
        ...
        <div class="muted">Hinweis: GRID/TILE werden aktuell nicht verändert.</div>
      </details>
